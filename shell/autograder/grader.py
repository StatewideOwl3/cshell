...
def main(
    ...
    if roster_path is not None:
        ...
        grade_student(log_dir)
    else:
        log_dirs = [
            p
            for p in logging_dir.iterdir()
            if p.is_dir() and (p / "tests").exists() and (p / "tests").is_dir()
        ]

        if not log_dirs:
            logger.warning(
                "No valid logs found in logging directory (looking for subfolders with /tests)"
            )
            raise typer.Exit(code=0)

        logger.info(f"Found {len(log_dirs)} student logs to grade")

        with concurrent.futures.ThreadPoolExecutor(max_workers=threads) as executor:
            future_to_log = {
                executor.submit(grade_student, log_dir): log_dir for log_dir in log_dirs
            }

            for _ in tqdm(
                concurrent.futures.as_completed(future_to_log),
                total=len(log_dirs),
                desc="Grading students"
            ):
                pass
